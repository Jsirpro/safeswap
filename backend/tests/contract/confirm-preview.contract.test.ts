...
async function createPreview(app: ReturnType<typeof buildApp>) {
...
describe('POST /api/intents/{intentId}/confirm contract', () => {
  ...
  it('returns execution permission and wallet payload only after preview confirmation', async () => {
    const { intentId, previewId } = await createPreview(app);
    ...
    expect(response.body.walletPayload).toMatchObject({
      intentId,
      previewId,
